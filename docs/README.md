...
* ENTRYPOINT
为了保证容器能够持续运行，ENTRYPOINT启动进程需要是一个常驻进程，否则容器会运行后立即退出。它有两种模式,shell模式:ENTRYPOINT ["Invoke-Expression $env:MSBUILD_ENV_SET;powershell.exe -NoExit -ExecutionPolicy ByPass"];exec模式:ENTRYPOINT ["powershell.exe", "Invoke-Expression", "$env:MSBUILD_ENV_SET", ";", "powershell.exe", "-NoExit", "-ExecutionPolicy", "ByPass"]
